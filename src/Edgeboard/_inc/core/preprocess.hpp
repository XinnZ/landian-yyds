...
#include <paddle_lite/paddle_image_preprocess.h>
...
// AI 模型图像预处理函数, 这个函数有predictor类接口进行调用, 用户不必直接使用它!
inline void fpga_preprocess(cv::Mat img, std::shared_ptr<ModelConfig> &config,
    ...
    if (height > 1080) {
        ...
        Mat resize_mat;
        resize(img, resize_mat, Size(config->input_width, config->input_height), fx, fy);
