...
struct option {
    option(std::string sname, std::string lname, std::string help,
...
class argparser {
   ...
   public:
    ...
    argparser &add_help_option() {
        return add_sc_option("-?", "--help", "show this help message",
                             [this]() { print_help(); });
    }

    // add short circuit option
    argparser &add_sc_option(std::string sname, std::string lname,
                             std::string help,
                             std::function<void(void)> callback) {
        ...
        short_circuit_options.emplace_back(std::move(sname), std::move(lname),
                                           std::move(help), std::move(callback));
        return *this;
    }
    ...
    argparser &add_option(std::string sname, std::string lname, std::string help,
        ...
        if (type_string<T>() == "null") {
            std::cerr << "(build error) unsupport type for option: "
        ...
        check_add_option_lname(lname);
        options.emplace_back(std::move(sname), std::move(lname), std::move(help),
                             type_string<T>(), to_string(default_value));
        return *this;
    }
    ...
    argparser &add_named_argument(std::string name, std::string help) {
        check_add_argument_name<T>(name);
        named_arguments.emplace_back(std::move(name), std::move(help),
                                     type_string<T>());
        return *this;
    }
    ...
    bool get_option_bool(const std::string &name) const {
        return get_option<bool>(name);
    }
    int get_option_int(const std::string &name) const {
        return get_option<int>(name);
    }
    int64_t get_option_int64(const std::string &name) const {
        return get_option<int64_t>(name);
    }
    double get_option_double(const std::string &name) const {
    ...
    int get_argument_int(const std::string &name) const {
        return get_argument<int>(name);
    }
    int64_t get_argument_int64(const std::string &name) const {
        return get_argument<int64_t>(name);
    }
    double get_argument_double(const std::string &name) const {
        return get_argument<double>(name);
    }
    std::string get_argument_string(const std::string &name) const {
    ...
    argparser &parse(int argc, char const *argv[]) {
        ...
        for (auto &&sc_opt : short_circuit_options) {
            auto pos = std::find_if(
                tokens.cbegin(), tokens.cend(), [&sc_opt](const std::string &tok) {
        ...
        for (auto &&opt : options) {
            auto pos = std::find_if(
                tokens.cbegin(), tokens.cend(), [&opt](const std::string &tok) {
            ...
            {
                if (pos == tokens.cend()) {
                    std::cerr << "(parse error) option " << opt.short_name << " "
                              << opt.long_name << " should have value" << std::endl;
                    std::exit(-1);
                }
                if (pos->front() == '-') {
                    std::cerr << "(parse error) option " << opt.short_name << " "
                              << opt.long_name << " followed by another option"
                              << std::endl;
                    std::exit(-1);
                }
                opt.value = *pos;
        ...
        // if there are short name like options, parse it as aggregation short name
        // options
        {
            auto pos = std::find_if(
                tokens.cbegin(), tokens.cend(),
                [](const std::string &tok) { return tok.front() == '-'; });
            if (pos != tokens.cend()) {
                if (pos->length() == 1) {
                ...
                if ((*pos)[1] == '-') {
                    std::cerr << "(parse error) unrecognized option" << (*pos)
                              << std::endl;
                    std::exit(-1);
                }
                ...
                for (char ch : short_names) {
                    ...
                    if (index < options.size() &&
                        options[index].short_name.back() == ch) {
                        if (options[index].type != "bool") {
                            std::cerr
                                << "(parse error) aggregation short name option must be bool"
                                << std::endl;
                            std::exit(-1);
                        }
        ...
        for (auto &named_arg : named_arguments) {
            ...
            if (named_arg.value == "") {
                std::cerr << "(parse error) named_argument " << named_arg.name
        ...
        if (tokens.size() != arguments.size()) {
            std::cerr << "(parse error) position argument number missmatching, give "
                      << tokens.size() << ", but need " << arguments.size()
                      << std::endl;
            std::exit(-1);
        }
   ...
   private:
    ...
    auto find_named_argument(const std::string &key) const -> argument_iterator {
        return std::find_if(
            named_arguments.cbegin(), named_arguments.cend(),
            [&key](const argument &arg) { return arg.name == key; });
    }

    auto find_sc_option_sname(const std::string &key) const
        -> sc_option_iterator {
        return std::find_if(short_circuit_options.cbegin(),
                            short_circuit_options.cend(),
                            [&key](const short_circuit_option &opt) {
                                return opt.short_name == key;
                            });
    }

    auto find_sc_option_lname(const std::string &key) const
        -> sc_option_iterator {
        return std::find_if(short_circuit_options.cbegin(),
                            short_circuit_options.cend(),
                            [&key](const short_circuit_option &opt) {
                                return opt.long_name == key;
                            });
    }
    ...
    void check_add_option_sname(const std::string &key) const {
        if (key.size() != 2 || key.front() != '-') {
            std::cerr << "(build error) short option name must be `-` followed by "
                         "one character"
                      << std::endl;
            std::exit(-1);
        }
        ...
        if (short_name_index.find(ch) != short_name_index.end()) {
            std::cerr << "(build error) short option name " << key
    ...
    void check_add_option_lname(const std::string &key) const {
        if (key == "") {
            std::cerr << "(build error) long option name cannot be empty"
                      << std::endl;
            std::exit(-1);
        }
        if (key.substr(0, 2) != "--") {
            std::cerr << "(build error) long option name must be `--` followed by "
    ...
    void check_add_argument_name(const std::string &key) const {
        if (type_string<T>() == "null") {
            std::cerr << "(build error) argument type is not supported: "
                      << typeid(T).name() << std::endl;
            std::exit(-1);
        }
        ...
        if (find_argument(key) != arguments.cend() ||
            find_named_argument(key) != named_arguments.cend()) {
            std::cerr << "(build error) argument name " << key << " already exists"
                      << std::endl;
            std::exit(-1);
        }
    }
