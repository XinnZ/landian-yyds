...
scuRcuResetCode evaluateReset(void) {
    ...
    if (rstStat.B.PORST) {
        ...
        if (resetCode.resetType != IfxScuRcu_ResetType_coldpoweron) {
            resetCode.resetType =
                IfxScuRcu_ResetType_warmpoweron; /* ResetType is Warm Power-On
                                                  */
            resetCode.resetTrigger =
                IfxScuRcu_Trigger_porst; /* Rest Trigger is PORST        */
        }
    }
...
/* This function gets information about the last occurred reset and turns on an */
...
void configureSwResetRequestTrigger(scuRcuResetType resetType) {
    ...
    if (resetType == IfxScuRcu_ResetType_application) {
        SCU_RSTCON.B.SW =
            IfxScuRcu_ResetType_application; /* Trigger SW Reset Type as
                                                application reset    */
    } else if (resetType == IfxScuRcu_ResetType_system) {
        SCU_RSTCON.B.SW = IfxScuRcu_ResetType_system; /* Trigger SW Reset Type
                                                         as system reset */
    }
...
/* This function triggers either a SW Application Reset or a SW System Reset,
 * based on the parameter resetType */
